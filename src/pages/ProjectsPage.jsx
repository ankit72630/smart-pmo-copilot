...
export default function ProjectsPage() {
  ...
  const kpis = [
    { label: "Total Projects", value: total, icon: FiLayers, accent: "text-blue-600" },
    { label: "At Risk", value: atRisk, icon: FiAlertTriangle, accent: "text-red-500" },
    { label: "Blocked", value: blocked, icon: FiSlash, accent: "text-yellow-600" },
    { label: "Avg. Velocity", value: `${avgVel} t/s`, icon: FiTrendingUp, accent: "text-green-600" },
    { label: "Avg. Cycle Time", value: `${avgCt} d`, icon: FiClock, accent: "text-purple-600" },
    { label: "Overall Health", value: "75%", icon: FiActivity, accent: "text-teal-600" },
  ];
  ...
  return (
    <div className="space-y-8">
        ...
        <table className="table-auto w-full text-left">
          <thead className="bg-gray-50 sticky top-0 z-10">
            <tr>
              ...
              <th className="px-4 py-2 font-medium text-gray-700 whitespace-nowrap">Status</th>
              <th className="px-4 py-2 font-medium text-gray-700">AI Summary</th>
              <th className="px-4 py-2 font-medium text-gray-700 whitespace-nowrap">Risk Score</th>
              <th className="px-4 py-2 font-medium text-gray-700 whitespace-nowrap">% Complete</th>
              <th className="px-4 py-2 font-medium text-gray-700 hidden lg:table-cell whitespace-nowrap">
                Next Milestone
              </th>
              <th className="px-4 py-2 font-medium text-gray-700 hidden lg:table-cell whitespace-nowrap">
                Last Updated
              </th>
              <th className="px-4 py-2 font-medium text-gray-700 text-center whitespace-nowrap">
                Actions
              </th>
            </tr>
          </thead>
            ...
            {pageData.length === 0 ? (
              ...
              pageData.map((p) => (
                ...
                >
                  ...
                  <td className="px-4 py-2 text-sm line-clamp-2">{p.summary}</td>
                  <td className="px-4 py-2 whitespace-nowrap">{p.riskScore}</td>
                  <td className="px-4 py-2 whitespace-nowrap">
                    <div className="w-full bg-gray-200 rounded-full h-2 overflow-hidden">
                      <div
                        className="h-2 bg-primary"
                        style={{ width: `${p.completePct * 100}%` }}
                      />
                    </div>
                  ...
                  <td className="px-4 py-2 text-center whitespace-nowrap flex gap-2">
                    <button
                      onClick={(e) => {
                        e.stopPropagation();
                        openChat(p);
      ...
      {/* Chat Drawer */}
      <ChatDrawer open={chatOpen} project={chatProject} onClose={() => setChatOpen(false)} />
    </div>
  );
}
